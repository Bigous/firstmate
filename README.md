...
Codex and OpenCode are also verified and supported as primary harnesses; Codex CLI 0.154.0+ uses native asynchronous Stop supervision, and OpenCode uses a TUI plugin, so both carry more harness-specific supervision tradeoffs than the three co-primaries.
