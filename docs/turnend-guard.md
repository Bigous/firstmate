...
Codex uses the bounded native queue hand-off proof owned by [`watcher-continuity.md`](watcher-continuity.md#codex-native-stop-ownership).
...
- Codex registers a `Stop` hook in `.codex/hooks.json`, anchors the executable to the hook process working directory, verifies a Firstmate-shaped hook-bearing root, and passes the original payload to the shared guard in `--codex` mode.
  Its native asynchronous Stop owner and exact callback proof are defined in [`watcher-continuity.md`](watcher-continuity.md#codex-native-stop-ownership).
